...
class RevVPSDE(torch.nn.Module):
    ...
    def vpsde_fn(self, t, x):
        ...
        # Ensure that both disc_steps and self.discrete_betas are on the same device as x
        disc_steps = disc_steps.to(device=x.device)
        discrete_betas_device = self.discrete_betas.to(device=x.device)
        
        beta_t = discrete_betas_device[disc_steps] * self.N # debug: using discrete params
    ...
    def rvpsde_fn(self, t, x, return_type='drift'):
        ...
        if return_type == 'drift':
            ...
            if disc_steps.unique() > 0:
                ...
                disc_steps = disc_steps.to(device=x.device)
                alphas_cumprod_device = self.alphas_cumprod.to(device=x.device)
                
                scale_factor = torch.sqrt(1 - alphas_cumprod_device[disc_steps - 1]) / torch.sqrt(1 - alphas_cumprod_device[disc_steps])
